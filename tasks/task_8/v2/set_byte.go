...
-------------------------------------------------------------------------------------
2 необычный способ - через массив рун
*/
...
func main() {
	...
	res, err := funcName(num, 3, true) // вызываем функцию, передаем исходное число, порядкоый номер бита для изменения и булево значения для вставки
	if err != nil {
		return
	}
	fmt.Println(res)
...
func funcName(num int64, order uint8, val bool) (int64, error) {
	if order > 64 {
